...
The MkDocs site is generated by CI/CD. Agents do not need to run MkDocs locally
for documentation changes; local preview is optional. On-demand commands are
...
9. During the release, generate combined docs for LLM upload (see below)
...
`docs/notebooklm-export.md` is generated during the release process. Do not edit
or regenerate this file during development; make changes in the source
documentation and let the release process refresh the export.
...
**Last Research Review**: _2026-09-15_ *(Update this date after completing twice-monthly review)*

**Review Summary** *(Keep last 3 months)*:
- **2026-09-15**: All scanners reviewed. Prior secrets enhancement #2185 confirmed CLOSED/shipped; postgres `postgresql://` issue #2322 still OPEN. **PI/CP**: No new regex-detectable patterns — Hermes repo had only infra commit (per-repo installation), no security payloads; all 10 existing Hermes payloads map to current ai-guardian scanners. OWASP LLM Top 10 unchanged since August. Notable research: XOXO cross-origin context poisoning via semantics-preserving code transforms (ACL 2026), Policy Simulation Jailbreak via XML/JSON structure mimicry, MCPTox benchmark (36.5% attack success across 353 MCP tools), Agentjacking via error-tracking platforms, CamoLeak via GitHub image proxy exfil — all architectural/compositional, not regex-detectable. CrowdStrike expanded PI taxonomy to 200+ techniques. AI credential leaks up 81% YoY (GitGuardian 2026). **Secrets**: Gitleaks still dormant (no config changes). LeakTK had pattern tuning only (flags, no new credential types). Betterleaks (MIT) added Bitbucket Data Center token (`BBDC-` prefix, PR #346). TruffleHog (AGPL, discovery only) added Resend (`re_`, independently verified from public docs) and Humio/LogScale detectors. Cross-referencing LeakTK (MIT) against secrets.toml identified 5 additional gaps: Shopify tokens (`shpat_`/`shpca_`/`shppa_`/`shpss_`), GitLab Pipeline Trigger (`glptt-`), GitLab Runner Registration (`glrt-`), OpenShift User Token (`sha256~`), Dynatrace (`dt0...`). Created issue #2323 for 7 new credential formats. **SSRF**: No new findings. **Config exfil**: No new findings. **Supply chain**: No new regex-detectable patterns. **Offensive lang**: No new findings. **Canary**: No new token formats. **Exfil detection**: No new techniques. See issue #2313.
