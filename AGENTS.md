...
**Last Research Review**: _2026-09-01_ *(See issue #2171)*
...
**Last Research Review**: _2026-09-01_ *(Update this date after completing twice-monthly review)*

**Review Summary** *(Keep last 3 months)*:
- **2026-09-01**: All scanners reviewed. Prior secrets enhancement #1986 confirmed CLOSED/shipped. **PI/CP**: No new regex-detectable patterns — Hermes had no security-payload changes, and OWASP GenAI LLM Top 10 had no changes since the August 15 review. New research (ECLIPSE, ICoA, LongPIBench, SkillGuard) highlights self-evolving long-horizon attacks, covert indirect prompt injection, long-context weakness, and capability confinement; these are architectural/compositional findings rather than safe static signatures. **Secrets**: Gitleaks and LeakTK had no pattern changes. Betterleaks (MIT) added six high-relevance AI-service credential formats absent from AI Guardian: Voyage AI, fal.ai, Cartesia, LlamaCloud, Mem0, and Retell AI — created issue #2185. TruffleHog added a Weights & Biases v2 detector, but its AGPL-3.0 implementation was used for discovery only; the candidate was deferred until its format can be independently verified from a compatible source. Other Betterleaks additions were generic/contextual service credentials or optimizations and were not prioritized over the AI-specific set. **SSRF**: No new endpoints or bypass techniques. **Config exfil**: No new paths or formats. **Supply chain**: No new regex-detectable patterns. **Offensive lang**: No new findings. **Canary**: No new token formats. **Exfil detection**: No new techniques. See issue #2171.
- **2026-08-15**: All scanners reviewed. All issues from previous review (#1777, #1778, #1779) confirmed CLOSED/shipped. **PI/CP**: No new regex-detectable patterns — Hermes repo dormant on security payloads (only infra commits: scaffold install, website build fix). OWASP GenAI LLM Top 10 2026 published Aug 4 — major reshuffle: Hidden Context Exposure (LLM08, renamed from System Prompt Leakage), Excessive Agency promoted to #3, cross-modal PI in images/audio (LLM01). Agentic Top 10 unchanged (Dec 2025). All architectural categories, not regex-detectable. CVE activity: Claude Code 50-subcommand bypass (patched 2.1.90), SOCKS5 hostname null-byte injection (CVE-2025-66479); Agentjacking (MCP data as instructions); Friendly Fire (AI Now, agent autonomous mode hijack) — all architectural. **Secrets**: 4 new credential formats — Cloudflare scannable tokens (`cfut_`/`cfat_`/`cfk_`, Feb 2026), Figma PAT v3 (`figp_`), GitLab Incoming Mail Token (`glimt-`, Betterleaks MIT), Salesforce Marketing Cloud (`SFMC_`, March 2026) — created issue #1986. TruffleHog added SolarWinds Observability (no prefix, keyword-only — rejected for FP risk), Figma v3, MS Teams Webhook V2 (AGPL-3.0; Figma independently authored). Betterleaks added GitLab incoming mail token and confidence/components metadata. Gitleaks still dormant (last config commit 2025-11-20). LeakTK no new commits since last review. **SSRF**: No new findings. **Config exfil**: No new findings. **Supply chain**: No new regex-detectable patterns. **Offensive lang**: No new findings. **Canary**: No new token formats. **Exfil detection**: No new techniques. See issue #1985.
