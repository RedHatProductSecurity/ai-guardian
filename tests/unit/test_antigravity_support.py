...
class TestArgumentCasing:
    """Every layer must resolve arguments identically, whatever the casing.

    A layer that misses a value while its neighbours resolve it skips its
    own check silently rather than failing.
    """

    def test_mcp_names_resolve_in_either_casing(self):
        from ai_guardian.constants import antigravity_tool_name

        pascal = antigravity_tool_name(
            "call_mcp_tool", {"ServerName": "srv", "ToolName": "tool"}
        )
        camel = antigravity_tool_name(
            "call_mcp_tool", {"serverName": "srv", "toolName": "tool"}
        )
        assert pascal == camel == "mcp__srv__tool"

    def test_command_resolves_in_either_casing(self):
        adapter = AntigravityAdapter()
        for key in ("CommandLine", "commandLine", "command_line"):
            n = adapter.normalize_input(_pre_tool_use("run_command", {key: "ls -la"}))
            assert n.tool_input["command"] == "ls -la", key

    def test_replacement_chunks_resolve_in_either_casing(self):
        adapter = AntigravityAdapter()
        for key in ("ReplacementChunks", "replacementChunks"):
            n = adapter.normalize_input(
                _pre_tool_use(
                    "propose_code",
                    {"TargetFile": "/w/a.py", key: [{"targetContent": "x = 1"}]},
                )
            )
            assert n.tool_input["content"] == "x = 1", key

    def test_arg_helper_tolerates_non_dict(self):
        from ai_guardian.constants import antigravity_arg

        assert antigravity_arg(None, "Any") is None
        assert antigravity_arg("not-a-dict", "Any") is None
        assert antigravity_arg({1: "x"}, "Any") is None
            ...
            json.dumps(
                {
                    "ai-guardian": {
                        ...
                        "PreInvocation": [{"command": "/usr/bin/ai-guardian"}],
                    }
                }
            )
        )
