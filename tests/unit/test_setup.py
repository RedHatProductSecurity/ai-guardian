...
class TestIDESetupParametrized:
    ...
    @staticmethod
    def _complete_managed_config(ide_name):
        """Build a config containing every event in the central manifest."""
        setup = IDESetup()
        template = deepcopy(setup.IDE_CONFIGS[ide_name]["hooks"])
        if ide_name == "copilot":
            return template
        return setup.merge_hooks({}, template, ide_name)[0]

    # Partial config data used by overwrite/reconciliation tests. Complete
    # managed manifests are generated by _complete_managed_config below.
    _CONFIGURED_CONFIGS = {
        "claude": {
            "hooks": {
                "UserPromptSubmit": [
    ...
    def test_managed_manifest_matches_every_setup_template(self):
        """Every integration's setup template follows the central manifest."""
        setup = IDESetup()

        for ide_name, expected_events in MANAGED_HOOK_EVENTS_BY_IDE.items():
            config = setup.IDE_CONFIGS[ide_name]
            assert tuple(setup.expected_hook_manifest(ide_name)) == (
                ("plugin",)
                if config.get("plugin_file")
                else (
                    ("extension",) if config.get("extension_based") else expected_events
                )
            )

            if (
                config.get("mcp_only")
                or config.get("plugin_file")
                or config.get("extension_based")
            ):
                continue
            if config.get("script_based"):
                configured_events = tuple(config["hook_scripts"])
            elif ide_name == "gemini":
                configured_events = tuple(
                    entry["event"]
                    for entry in config["hooks"]["hooks"]
    ...
    def test_check_hooks_configured_json(self, tmp_path, ide_name):
        ...
        config_file.write_text(json.dumps(self._complete_managed_config(ide_name)))
    ...
    def test_check_hooks_configured_script(self, tmp_path, ide_name):
        ...
        for script_name in setup.expected_hook_manifest(ide_name):
...
class TestCursorSetup:
    ...
    def test_cursor_setup_writes_explicit_project_scope(self, tmp_path):
        """Explicit Cursor project setup writes only the selected workspace."""
        setup = IDESetup()
        user_hooks = tmp_path / "user" / "hooks.json"
        project_hooks = tmp_path / "project" / ".cursor" / "hooks.json"
        ...
        with mock.patch.object(
            ...
            with mock.patch(
                ...
                success, _ = setup.setup_ide_hooks(
                    ...
                    force=True,
                    scope="project",
                    project_dir=str(project_hooks.parent.parent),
                )
    ...
    def test_cursor_effective_health_still_requires_user_scope(self, tmp_path):
        ...
        project_mcp = tmp_path / "project" / ".cursor" / "mcp.json"
        project_mcp.write_text(
            json.dumps({"mcpServers": {"ai-guardian": {"command": "other"}}})
        )

        def hook_path(_ide, scope="user", project_dir=None):
            return str(user_hooks if scope == "user" else project_hooks)

        def mcp_path(_ide, scope="user", project_dir=None):
            return user_mcp if scope == "user" else project_mcp

        with (
            mock.patch.object(setup, "get_config_path", side_effect=hook_path),
            mock.patch(
                "ai_guardian.setup.mcp.get_mcp_config_path", side_effect=mcp_path
            ),
            mock.patch.object(
                setup, "_cursor_project_root", return_value=project_hooks.parent.parent
            ),
        ):
            verification = setup.verify_ide_setup("cursor")

        assert verification["healthy"] is False
        assert verification["installation_scope"] == "user"
        assert verification["effective_scope"] == "project"
        assert verification["mcp_installed"] is False
        assert verification["effective_mcp_installed"] is True
...
class TestSetupHooks:
    ...
    def test_setup_hooks_cursor_project_target(self, tmp_path):
        """Project setup forwards the selected workspace to hooks and MCP."""
        ...
        assert success is True
        expected_project = str(project_dir.resolve())
        mock_instance.setup_ide_hooks.assert_called_once_with(
            "cursor",
            dry_run=False,
            force=False,
            scope="project",
            project_dir=expected_project,
        )
...
class TestAbsolutePathWritten:
    ...
    def test_check_hooks_configured_recognizes_absolute_path(self, tmp_path):
        ...
        managed_hooks = {
            ...
            for event_name in setup.expected_hook_manifest("claude")
        }
        config_file.write_text(json.dumps({"hooks": managed_hooks}))
...
class TestWindowsSetup:
    ...
    def test_is_already_configured_detects_bat(self, tmp_path):
        ...
        hooks_dir = tmp_path / "hooks"
        hooks_dir.mkdir()
        for script_name in setup.expected_hook_manifest("cline"):
            (hooks_dir / f"{script_name}.bat").write_text(
                "@echo off\r\nai-guardian --ide cline\r\n"
            )
