...
class TestCheckHooks:
    ...
    def test_cursor_reports_user_scope_and_effective_project_scope(
        ...
        """Doctor distinguishes the install target from project discovery."""
        ...
        verification = {
            ...
            "events": {
                event: "missing"
                for event in IDESetup().expected_hook_manifest("cursor")
            },
            "obsolete": [],
        }
        ...
        with (
            ...
            mock.patch(
                "ai_guardian.setup.IDESetup.get_config_path",
                side_effect=lambda ide_type, scope="user", project_dir=None: str(
                    user_hooks if scope != "project" else project_hooks
                ),
            ),
            mock.patch(
                "ai_guardian.setup.IDESetup.get_cursor_config_layers",
                return_value=[
                    {"scope": "user", "path": str(user_hooks), "exists": False},
                    {
                        "scope": "project",
        ...
        assert result.status == CheckStatus.WARN
        assert "scope: user; effective: project" in result.message
        assert "MCP: missing" in result.message
    ...
    def test_codex_hooks_use_managed_event_count(self, _isolate_config_dir, tmp_path):
        ...
        hooks = {
            name: [
                {"hooks": [{"type": "command", "command": "ai-guardian --ide codex"}]}
            ]
        ...
        with (
            mock.patch(
                "ai_guardian.setup.IDESetup.list_detected_ides", return_value=["codex"]
            ),
            mock.patch(
                "ai_guardian.setup.IDESetup.get_config_path",
